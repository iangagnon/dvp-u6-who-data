...
new_col_name = 'Life expectancy at birth (years)'
df.rename(columns = {'Life expectancy at birth (years)':new_col_name})
...
sns.barplot(data=df, x='Country', y= new_col_name)
plt.show()
...
GDP does not corelate with LEABY. No.
...
sns.violinplot(data=df, x='Country', y=new_col_name)
...
f, ax = plt.subplots(figsize=(10, 15)) 
ax = sns.barplot(data=df, x='Country', y=new_col_name, hue='Year')
plt.xticks(rotation=90)
...
China and the US had large increases in GDP. Zimbabwe and China had the greatist increase in life expectancy at birth.
2008-2015
Zimbabwe
Some have large changes others have small changes.
It is interesting the Zimbabwe's large increas in life expectancy was not due to a large increase in GDP. The two relationships are not always corelated.
Perhaps technology has enabled Zimbabwe to increase its life expectancy and that technology does not depend on a high GDP in order to use it.
...
g = (g.map_dataframe(plt.scatter, x='GDP', y=new_col_name, edgecolor="w").add_legend())
plt.show()
...
China
Zimbabwe
The previous charts showed these same trends, but it is odd that Zimbabwe's life expectancy at birth increases but their GDP does not also increase.
Perhaps a video would show this better with each year being one frame.
...
g3 = (g3.map_dataframe(plt.plot, 'Year', new_col_name).add_legend())
plt.show()


# What are your first impressions looking at the visualized data?
# Zimbabwe had the greatest improvement in Life Expectancy, but also started the lowest.
# 
...
# Mexico
# - Can you think of any reasons that the data looks like this for particular countries?
# Zimbabwe being a less developed country had lots of room for improvement by implementing healthcare strategies used in the countries whoes life expectancies did not increase as much.
...
Highest: United States of America
Lowest: Zimbabwe and Chile
...
Highest:Chile, Germany, and The United States of America
Lowest: Zimbabwe
...
Zimbabwe has suffered from hyperinflation. In 2009 they legalized the use of foreign currencies. This enabled the price of goods to stabalize, but has probably not helped to increase their GDP.
